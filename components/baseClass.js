...
export default class rizPluginBase extends plugin {
  ...
  async getMicInfoFromMsg(e, fncName, params = [], options, callback) {
    ...
    if (!ids.length) {
      send.send_with_At(e, `未找到${msg}的相关曲目信息QAQ！如果想要提供别名的话请访问 /rizhelp 中的别名投稿链接嗷！`, true)
      return;
    }
    ...
    if (ids.length > 1) {
      this.choseMutiNick(e, ids, { ...optObj, ...options }, (e, id, optObj) => {
        ...
        if (!info) {
          ...
          logger.error(`riz-plugin错误：未找到曲目ID为${id}的曲目信息！`)
          return;
        }
        if ('rank' in optObj) {
          const { rank } = optObj
          if (!info.chart[/** @type {levelKind} */(rank)]) {
            send.send_with_At(e, `${info.name} 没有 ${rank} 这个难度QAQ！`)
            return;
          }
        }
