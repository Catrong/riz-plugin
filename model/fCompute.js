...
export default class fCompute {
     ...
     * @param {number} rizHit Riztime时段总Hit
     * @param {number} rizPerfect Riztime时段Perfect数量
     * @param {number} hit 获得的Hit数量
     * @param {number} totHit 总Hit数量
     * @returns 
     */
    static rks(acc, difficulty, level, rizHit, rizPerfect, hit, totHit) {
        let rizRks = 0;
        let k1 = 0;
        switch (level) {
            case 'EZ':
        ...
        rizRks = difficulty * Math.min(rizPerfect, hH) / hH;
        ...
        if (hit > totHit / 2) {
            hitRks = 10 * difficulty * (2 * hit - totHit) / totHit;
        }
        return rizRks + accRks + hitRks;
