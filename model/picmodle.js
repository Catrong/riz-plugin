...
export default await new class picmodle {
    ...
    async init() {
        ...
        try {
            ...
            logger.error(`[riz-Plugin][清理临时文件失败]`)
            logger.error(err)
        }
        /** 初始化puppeteer实例 */
    ...
    /** 
     * @typedef {'atlas'|'table'|'b40'} picKind
     */
    ...
    async render(path, params, cfg) {
                ...
                try {
                    ...
                    let data = {
                        ...
                        sys: {
                            ...
                            copyright: `Created By Yunzai-Bot<span class="version">${Version.yunzai}</span> & riz-Plugin<span class="version">${Version.ver}</span>`
                        },
                        Version: { ...Version },
                    ...
                    logger.error(`[riz-Plugin][渲染失败]`, id)
                    logger.error(err)
                    logger.warn(`[riz-Plugin][渲染器]`, puppeteerNum)
                    logger.warn(`[riz-Plugin][空闲渲染器队列]`, this.queue)
                    logger.warn(`[riz-Plugin][渲染队列] `, this.rendering)
                    logger.warn(`[riz-Plugin][等待队列] `, this.torender)
                    ans = '渲染失败QAQ！\n' + err
                }
        ...
        if (!ans) {
            ans = '等待超时，请稍后重试QAQ！'
            logger.error(`[riz-Plugin][等待超时]`, id)
            logger.warn(`[riz-Plugin][空闲渲染器队列]`, this.queue)
            logger.warn(`[riz-Plugin][渲染队列] `, this.rendering)
            logger.warn(`[riz-Plugin][等待队列] `, this.torender)
